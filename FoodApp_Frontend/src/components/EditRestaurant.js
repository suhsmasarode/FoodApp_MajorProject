...
export default function EditRestaurant() {
    ...
    const restaurant=loc.state;
    const [restroName, setRestroName] = useState(restaurant.restroName);
    const [location, setLocation] = useState(restaurant.location);
    
    console.log("restaurant User id is :"+JSON.stringify(restaurant.user.id))
   ...
   // console.log("email is " + data);
    ...
    const baseURL = "http://localhost:8080/updateRestaurant/"+restaurant.restroId;
  
    const onSubmit = (e) => {
      createPost();
    };
    const formData={restroName:restroName,location:location,user:restaurant.user}
